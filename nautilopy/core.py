...
# %% auto 0
__all__ = ['cli', 'v_quat_example', 'd_roll_ex', 'd_pitch_ex', 'd_yaw_ex', 'v_quat_ex', 'f_template_function', 'f_q2rollPitchYaw',
           'f_test_f_q2rollPitchYaw', 'f_rollPitchYaw2q', 'f_test_f_rollPitchYaw2q', 'f_nav2target_pos', 'f_hyster',
           'f_save', 'f_load', 'f_generate_unique_filename', 'f_save_var', 'f_load_var', 'f_pointCloud',
           'f_plot_rotated_axes', 'f_plot_3d_trajectory', 'f_image_grid', 'f_plot_auv_orientation_subplots',
           'f_plot_auv_orientation_trajectory', 'f_create_auv_orientation_animation_plotly', 'f_add_trajectory_traces',
           'f_create_axes', 'f_add_bounding_box', 'f_add_initial_position_and_axes', 'f_add_animation_frames',
           'f_update_layout', 'f_create_auv_orientation_animation', 'f_plot_polar_sonar_seaking',
           'f_plot_polar_sonar_micron', 'f_plot_horizontal_sonar', 'f_plot_horizontal_sonar_with_angles',
           'f_plot_vertical_sonar_with_angles', 'f_animation_AUV_micron_trajectory', 'f_create_polar_sonar_animation',
           'f_micron_with_trajectory', 'f_seaking_with_trajectory', 'f_format_time', 'f_add_packages_to_all']
...
def f_plot_auv_orientation_subplots(m_pos_auv: np.ndarray, m_ypr: np.ndarray) -> None:
    """
    ...
    **Example**
    ...
    4. **AUV Orientation Visualization**:
       - For each subplot:
         a. Plots rotated axes at regular intervals along the trajectory using `f_plot_rotated_axes`.
         b. The axes represent the AUV's orientation (yaw, pitch, roll) at each point.

    5. **Trajectory Plotting**:
       - Plots the AUV's trajectory using `f_plot_3d_trajectory`.
    ...
    **References**  
    ...
    """
    # Data setup
    d_start = 9000  # Start index for data sampling
    d_end = 11100  # End index for data sampling
    d_step = 200  # Step size for data sampling
    d_scale = 1.5  # Scale factor for visualization

    # Get trajectory data and adjust depth values
    m_trajectory = m_pos_auv[d_start:d_end + 100, :].copy()
    m_trajectory[:, 2] = -m_trajectory[:, 2]

    # Set up the figure size
    h_fig_auv_orientation_subplots = plt.figure(figsize=(12, 12))

    # Loop through different view angles
    for d_i, t_view in enumerate([(30, -45), (0, 0), (0, 90), (90, 90)]):
        d_elev, d_azim = t_view
        # Add subplot for each view angle
        h_ax = h_fig_auv_orientation_subplots.add_subplot(2, 2, d_i + 1, projection="3d", proj_type="ortho")

        # Plot rotated axes for each step
        for d_ind in range(d_start, d_end, d_step):
            r_rotation = R.from_euler("ZYX", m_ypr[d_ind, :] + np.array([0, 0, np.pi]))
            f_plot_rotated_axes(h_ax, r_rotation, t_offset=(m_pos_auv[d_ind, 0], m_pos_auv[d_ind, 1], -m_pos_auv[d_ind, 2]), d_scale=d_scale)
...
def f_create_auv_orientation_animation_plotly(m_pos_auv: np.ndarray, m_ypr: np.ndarray, v_timestamp_nav: np.ndarray) -> go.Figure:
    """
    ...
    **References**  
    ...
    """
    d_start, d_end, d_step = 0, np.shape(m_pos_auv)[0], 50
    m_trajectory = m_pos_auv[d_start:d_end + 100, :].copy()
    m_trajectory[:, 2] = -m_trajectory[:, 2]
...
def f_create_auv_orientation_animation(m_pos_auv: np.ndarray, m_ypr: np.ndarray) -> FuncAnimation:
    """
    ...
    **References**  
    ...
    """
    # Define start, end, and step for data sampling
    d_start = 0  
    d_end = np.shape(m_pos_auv)[0]  
    d_step = 200

    # Get a trajectory and adjust depth values to negative
    m_trajectory = m_pos_auv[d_start:d_end + 100, :].copy()
    m_trajectory[:, 2] = -m_trajectory[:, 2]

    # Set up figure size and projection
    h_fig_auv_orientation_animation = plt.figure(figsize=(12, 9), dpi=100)
    h_ax = h_fig_auv_orientation_animation.add_subplot(projection="3d", proj_type="ortho", box_aspect=(5, 4, 2))
    h_ax.set_title("AUV orientation over trajectory", {'fontsize': 30})

    # Set initial view angle and axis inversion
    h_ax.view_init(30, 45)
    h_ax.dist = 0
    h_ax.invert_xaxis()

    # Calculate rotations for each step
    l_r = [R.from_euler("ZYX", m_ypr[d_ind, :] + np.array([0, 0, np.pi])) for d_ind in range(d_start, d_end, d_step)]

    def animate(d_num):
        # Clear previous AUV axes
        for h_line in h_ax.lines[-4:]:
            h_line.remove()
        for h_text in h_ax.texts[-4:]:
            h_text.remove()
        # Plot rotated AUV axes for current step
        f_plot_rotated_axes(h_ax, l_r[d_num], t_offset=(m_trajectory[d_num * d_step, 0], m_trajectory[d_num * d_step, 1], m_trajectory[d_num * d_step, 2]), d_scale=4)
        # Plot trajectory with start and end points
        h_ax.plot(m_trajectory[:, 0], m_trajectory[:, 1], m_trajectory[:, 2], label="Trajectory", color='purple')
        h_ax.scatter(m_trajectory[0, 0], m_trajectory[0, 1], m_trajectory[0, 2], color='green', label='Start')
        h_ax.scatter(m_trajectory[-1, 0], m_trajectory[-1, 1], m_trajectory[-1, 2], color='red', label='End')
        ...
        d_margin = 3
        h_ax.set_xlim3d([np.min(m_trajectory[:, 0]) - d_margin, np.max(m_trajectory[:, 0]) + d_margin])
        h_ax.set_xlabel('X')
        h_ax.set_ylim3d([np.min(m_trajectory[:, 1]) - d_margin, np.max(m_trajectory[:, 1]) + d_margin])
        h_ax.set_ylabel('Y')
        h_ax.set_zlim3d([np.min(m_trajectory[:, 2]) - d_margin, np.max(m_trajectory[:, 2]) + d_margin])
        h_ax.set_zlabel('Z')

    # Create animation and close figure to prevent showing initial plot
    anim = FuncAnimation(h_fig_auv_orientation_animation, animate, interval=200, frames=(d_end - d_start) // d_step, repeat=False)
    plt.close(h_fig_auv_orientation_animation)
    
    return anim

# %% ../nbs/00_core.ipynb 46
def f_plot_polar_sonar_seaking(m_sonar: np.ndarray, v_angle: np.ndarray) -> None:
    """
...
def f_plot_polar_sonar_micron(m_sonar: np.ndarray, v_angle: np.ndarray) -> None:
    """
...
def f_plot_horizontal_sonar(m_sonar: np.ndarray, d_start_micron: int, d_image_width: int, v_range_micron: np.ndarray) -> None:
    """
    ...
    **References**  
    ...
    # Create the figure and axes
    h_fig, h_ax = plt.subplots(figsize=(16, 9))

    # Plot the sonar data
    h_ax.set_title("Horizontal Sonar", {'fontsize': 24})
    h_ax.set_xlabel('1 revolution every 200 points', {'fontsize': 18})
    h_ax.set_ylabel('397 bins over 20 meters', {'fontsize': 18})
    h_ax.tick_params(axis='both', labelsize=18)
    h_im = h_ax.imshow(m_sonar, aspect='auto', interpolation='none', extent=[d_start_micron-194, d_end_micron, v_range_micron[-1], 0])
...
def f_plot_horizontal_sonar_with_angles(m_sonar: np.ndarray, v_angles_rad: np.ndarray, d_start_micron: int, d_image_width: int, v_range_micron: np.ndarray) -> None:
    """
    ...
    **References**  
    ...
    # Plot the sonar data
    h_ax.set_title("Horizontal Sonar", {'fontsize': 24})
    h_ax.set_xlabel('1 revolution (360 degrees) every 200 points', {'fontsize': 18})
    h_ax.set_ylabel('397 bins over 20 meters', {'fontsize': 18})
    h_ax.tick_params(axis='both', labelsize=18)
...
def f_plot_vertical_sonar_with_angles(m_sonar: np.ndarray, v_angles_rad: np.ndarray, d_start: int, d_image_width: int, v_range: np.ndarray) -> None:
    """
    ...
    **References**  
    ...
    # Plot the sonar data
    h_ax.set_title("Vertical Sonar", {'fontsize': 24})
    h_ax.set_xlabel('1 revolution (360 degrees) every 201 points', {'fontsize': 18})
    h_ax.set_ylabel('50 bins over 10 meters', {'fontsize': 18})
    h_ax.tick_params(axis='both', labelsize=18)
...
def f_animation_AUV_micron_trajectory(m_interp_pos_micron_final: np.ndarray, 
    ...
    **References**  
    ...
    # Load the image
    h_im = Image.open("./img/CavesGoogleEarth2.jpg")
    t_resize = (800, 600)
    h_im = h_im.resize(t_resize)
...
def f_add_packages_to_all() -> None:
    ...
    **References**  
    ...
    l_packages = [
        ...
        'pd', 'R', 'Slerp', 'scipy', 'HTML', 'display', 'disk', 'rank', 'rgb2gray', 'data', 
        'filters', 'cmd', 'cli', 'warnings', 'pytest', 'Tuple', 'Optional', 
        'List', 'Dict', 'Union', 'pickle', 'time', 'pio'
    ]
